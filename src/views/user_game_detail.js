...
exports.game_detail = (game, category, language, add_to_cart) =>{
...
exports.add_to_cart = (game) => {
    let result = ''
    if(store.get('key').id==null)
    {
        result += `
        <form class="cart clearfix">
            <a href="/login_user" class="btn amado-btn">Login</a>
        </form>
        `
    }
    else
    {
        result += `
        <form class="cart clearfix">
            <a href="/cartplus?user_id=${store.get('key').id}&company_id=${game[0].company_id}&game_name=${game[0].name}&amount=${game[0].price}" class="btn amado-btn">Add to cart</a>
        </form>
        `
    }
    return result;
